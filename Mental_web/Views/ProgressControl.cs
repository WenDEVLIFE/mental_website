    ...
    public class ProgressControl : UserControl
    {
        ...
        private void LoadData()
        {
            ...
            try
            {
                ...
                {
                    var assessments = db.SelfAssessments
                    ...
                    foreach (var ass in assessments) CreateProgressCard(ass);
                }
            }
            catch (Exception ex) { MessageBox.Show("Error loading progress: " + ex.Message); }
        }

        private void CreateProgressCard(SelfAssessment ass)
        {
            Panel card = new Panel { Size = new Size(710, 80), BackColor = Color.White, Margin = new Padding(0, 0, 0, 15) };
            UIHelper.MakeRounded(card, 15);

            var lblDate = new Label { Text = ass.DateTaken?.ToShortDateString() ?? "N/A", Font = new Font("Segoe UI", 10, FontStyle.Bold), Location = new Point(20, 30), AutoSize = true };
            var lblResult = new Label { Text = ass.Result ?? "Completed", Font = new Font("Segoe UI", 11), ForeColor = AppTheme.PrimaryColor, Location = new Point(150, 28), AutoSize = true };
            var lblScore = new Label { Text = $"Score: {ass.Score}%", Font = new Font("Segoe UI", 12, FontStyle.Bold), ForeColor = Color.DimGray, Location = new Point(550, 26), AutoSize = true, TextAlign = ContentAlignment.TopRight };

            var indicator = new Panel { Size = new Size(100, 10), Location = new Point(550, 55), BackColor = Color.FromArgb(240, 240, 240) };
            var bar = new Panel { Size = new Size((int)(ass.Score ?? 0), 10), Location = new Point(0, 0), BackColor = AppTheme.PrimaryColor };
